 ...
 * Description : SGFPLibSample main.cpp sample source code module
 * Copyright(c): 2015 SecuGen Corporation, All rights reserved
 * History : 
 *
 *************************************************************/
...
#if 1
  static long long ms_base = 0;

  gettimeofday(&tv, NULL);

  if (ms_base == 0)
  {
    ms_base = (tv.tv_sec*1000LL + tv.tv_usec/1000);
  }

  ms = (long)((tv.tv_sec*1000LL + tv.tv_usec/1000) - ms_base);
#else
...
DWORD ImageCallback(void *pUserData, void *pCallbackData)
{
  SGCBLiveCaptureParam *pInfo = (SGCBLiveCaptureParam*)pCallbackData;
...
print_time(function);
    ...
    sgfplib->SetCallBackFunction(CALLBACK_LIVE_CAPTURE, ImageCallback, NULL);
    //
    DWORD timeout = 35000; //5000 milliseconds
