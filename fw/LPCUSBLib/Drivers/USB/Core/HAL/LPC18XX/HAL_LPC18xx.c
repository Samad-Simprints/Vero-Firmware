...
IP_USBHS_001_T * const USB_REG_BASE_ADDR[LPC18_43_MAX_USB_CORE] = {(IP_USBHS_001_T*)LPC_USB0, (IP_USBHS_001_T*)LPC_USB1};
 ...
 *********************************************************************/
 void HAL_USBInit(uint8_t corenum)
 {
  ...
  if (!coreEnabled[corenum])
  {
    /* if neither core is currently enabled, then enable the USB PLL */
    if (!coreEnabled[1 - corenum])
    {
      /* Set up USB0 clock */
      /* Disable PLL first */
...
int USB_ReadBSessionValidState()
{
  return USB_REG(0)->OTGSC & (1 << 11);
}

int USB_ReadVBUSValidState()
{
  return USB_REG(0)->OTGSC & (1 << 9);
}


int USB_BsessionState = USB_MODE_None;
int USB_VBUSState = USB_MODE_None;

int USB_GetVBUSState(void)
{
  return USB_ReadVBUSValidState() ? USB_MODE_Device : USB_MODE_Host;
//	return USB_BsessionState[USBCore];
}

int USB_GetBsessionState()
{
  return USB_ReadBSessionValidState() ? USB_MODE_Device : USB_MODE_Host;
//	return USB_BsessionState[USBCore];
}


void CheckBsessionState()
{
  int otgsc = 0;

  otgsc = USB_REG(0)->OTGSC;

  if (otgsc & (1 << 19)) // B-session threshold crossed
  {
    if (USB_ReadBSessionValidState())
    {
      USB_BsessionState = USB_MODE_Device;
    }
    else
    {
      USB_BsessionState = USB_MODE_Host;
    }
  }
  USB_REG(0)->OTGSC = otgsc; // clear all interrupts
