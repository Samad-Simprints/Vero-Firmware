...
public class DemoConnection extends Connection
{
  ...
  public void writeMessage(int length, byte[] data)
  {
    ...
    switch(bCmd)
    {
      case Scanner.MSG_GET_SENSOR_INFO:
        ...
        reply.put(bdAddr);
        reply.putShort((short) 1); // iUCVersion
        reply.putShort((short) 3); // iUN20Version
        reply.putShort((short)50); // iBatteryLevel1
        reply.putShort((short)50); // iBatteryLevel2
        reply.putShort((short)0);  // iStoreCount
        reply.put((byte) 0);       // boPowerOn
  ...
  public void readMessage(int length, byte[] data, int offset)
  {
    ...
    Log.v(TAG, "Bytes read: " + length + ", offset: " + offset);
  }
